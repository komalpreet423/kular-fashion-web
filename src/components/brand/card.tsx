  ...
  const handleFavoriteToggle = async () => {
    ...
    if (user_id) {
      try {
        ...
        toast.error("Error adding to wishlist: " + error.message || error);
      } finally {
      }
    } else {
      // Your localStorage handling remains unchanged
      try {
        const wishlist = JSON.parse(localStorage.getItem("wishlist") || "[]");
        ...
        toast.error(
          "Error updating wishlist locally: " + error.message || error
        );
      } finally {
      }
    }
    ...
    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      <Card className="p-0 gap-2 shadow-md rounded-none overflow-hidden relative">
        <div className="relative overflow-hidden rounded-t-xl">
          ...
          {/* <motion.div className="absolute cursor-pointer top-3 right-3 bg-white p-2 rounded-full shadow-md cursor-pointer hover:bg-gray-200 z-10" whileTap={{ scale: 0.8 }} onClick={() => handleFavoriteToggle()} >
                            {isFavorited ?
